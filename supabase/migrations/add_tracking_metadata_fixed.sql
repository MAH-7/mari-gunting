...
-- Created: 2025-01-09
-- Fixed: 2025-10-15 (corrected enum values)
...
-- Add comments
COMMENT ON COLUMN bookings.tracking_started_at IS 'Timestamp when barber started traveling (status changed to accepted/confirmed)';
...
-- Function to auto-update tracking_started_at when status changes
CREATE OR REPLACE FUNCTION set_tracking_started_at()
RETURNS TRIGGER AS $$
BEGIN
  -- Set tracking_started_at when status changes to 'accepted' or 'confirmed'
  IF NEW.status IN ('accepted', 'confirmed') AND OLD.status NOT IN ('accepted', 'confirmed') THEN
    NEW.tracking_started_at = NOW();
  END IF;
  
  -- Set barber_arrived_at when status changes to 'in_progress'
