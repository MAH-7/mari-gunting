-- Update update_booking_status to handle refunds when booking is cancelled
-- This ensures refunds work for both customer and barber cancellations
...
BEGIN
  ...
  IF v_booking.status IS NULL THEN
    RETURN QUERY SELECT FALSE, 'Booking not found'::TEXT, NULL::TIMESTAMPTZ, FALSE, NULL::VARCHAR(255), NULL::NUMERIC;
    RETURN;
  END IF;
  ...
  -- No refund needed
  RETURN QUERY SELECT TRUE, 'Status updated successfully'::TEXT, v_updated_at, FALSE, NULL::VARCHAR(255), NULL::NUMERIC;
END;
$$;

COMMENT ON FUNCTION update_booking_status IS 'Updates booking status and handles refund when cancelled';
