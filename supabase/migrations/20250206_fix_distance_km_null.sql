-- ============================================
-- FIX: Travel fee formula + distance_km NULL
-- ============================================
-- ISSUE #1: Wrong travel fee formula (was RM0.50/km)
--           CORRECT: 0-4km = RM5, after 4km = RM5 + RM1/km
-- ISSUE #2: When customer address has no lat/lng,
--           v_distance_km stays NULL → "Distance N/A"
-- FIX: Correct travel formula + initialize distance_km
...
DECLARE
  ...
  v_distance_km NUMERIC := 0;  -- FIX: Initialize to 0 instead of NULL
  v_voucher RECORD;
BEGIN
  ...
  IF p_service_type = 'home_service' THEN
    ...
    IF p_customer_address IS NOT NULL AND p_customer_address->>'lat' IS NOT NULL THEN
      ...
      -- FIXED: Travel fee formula (0-4km = RM5 flat, after 4km = RM5 + RM1/km)
    ...
    ELSE
      -- FIX: Fallback - estimate 10km for missing location (travel fee RM 5)
      v_distance_km := 10.0;
      v_travel_fee := 5.00;
      RAISE NOTICE 'Missing customer coordinates, using estimated 10km / RM 5 travel fee';
    END IF;
  ELSE
...
COMMENT ON FUNCTION create_booking_v2 IS 
  'SECURITY: Validates service prices, calculates travel fees, applies vouchers server-side. FIX: Correct travel fee formula + distance_km always set.';
...
-- ✅ Fixed 2 issues:
--
-- 1. TRAVEL FEE FORMULA (CRITICAL FIX):
--    OLD: RM0.50/km, min RM3, max RM20 ❌ WRONG
--    NEW: 0-4km = RM5 flat, after 4km = RM5 + RM1/km ✅ CORRECT
--    Examples:
--      - 2 km → RM 5.00
--      - 4 km → RM 5.00  
--      - 6 km → RM 7.00 (5 + 2×1)
--      - 10 km → RM 11.00 (5 + 6×1)
--
-- 2. DISTANCE_KM NULL FIX:
--    - Initialize v_distance_km := 0 (was uninitialized)
--    - Set v_distance_km = 10.0 for home_service with missing coords
--    - Set v_distance_km = 0.0 explicitly for walk_in
--    Result: Partner dashboard shows distance instead of "N/A"
--
-- IMPACT: All new bookings will use correct travel fees
