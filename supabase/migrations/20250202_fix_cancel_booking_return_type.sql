-- Fix cancel_booking return type mismatch
-- The issue: payment_id column is VARCHAR but we were returning TEXT in NULL cases
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT FALSE, 'Booking not found or unauthorized', FALSE, FALSE, NULL::VARCHAR, NULL::NUMERIC;
    RETURN;
  END IF;

  -- Check if booking can be cancelled
  IF v_booking.status IN ('completed', 'cancelled') THEN
    RETURN QUERY SELECT FALSE, 'Cannot cancel completed or already cancelled booking', FALSE, FALSE, NULL::VARCHAR, NULL::NUMERIC;
    RETURN;
  END IF;
  ...
  IF v_booking.payment_status = 'authorized' 
    ...
    RETURN QUERY SELECT 
      ...
      FALSE, -- refund_needed
      TRUE,  -- reverse_needed
      v_booking.curlec_payment_id::VARCHAR,
      v_refund_amount;
    RETURN;
  ...
  ELSE
    ...
    RETURN QUERY SELECT TRUE, 'Booking cancelled successfully', FALSE, FALSE, NULL::VARCHAR, NULL::NUMERIC;
    RETURN;
  END IF;
